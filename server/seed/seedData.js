...
const seedData = async () => {
  try {
    ...
    // Do not clear existing data by default. This preserves registered users, admin data, orders, and products.

    // ─── Ensure Admin User Exists ───────────────────────────────────────────
    let admin = await User.findOne({ email: "admin@rentify.com" });
    if (!admin) {
    ...
    let demoUser = await User.findOne({ email: "user@rentify.com" });
    if (!demoUser) {
    ...
    if (existingProductCount === 0) {
      ...
      console.log(`📦 ${existingProductCount} products already exist, skipping seeding`);
    }
